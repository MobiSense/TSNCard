...
/* The user must tune the application number of channels to match the proxy
 * driver device tree and the names of each channel must match the dma-names in
 * the device tree for the proxy driver node. The number of channels can be less
 * than the number of names as the other channels will just not be used in
 * testing.
 */
...
const char *tx_channel_names[] = {"dma_proxy_tx_plc",
                                  /* add unique channel names here */};
const char *rx_channel_names[] = {"dma_proxy_rx_plc",
                                  /* add unique channel names here */};
...
int axi_dma_init() {
    /* Open the file descriptors for each tx channel and map the kernel driver
    ...
    for (i = 0; i < TX_CHANNEL_COUNT; i++) {
        ...
        tx_channels[i].buf_ptr = (struct channel_buffer *)mmap(
            NULL, sizeof(struct channel_buffer) * TX_BUFFER_COUNT,
    ...
    /* Open the file descriptors for each rx channel and map the kernel driver
     * memory into user space */
    for (i = 0; i < RX_CHANNEL_COUNT; i++) {
        ...
        rx_channels[i].buf_ptr = (struct channel_buffer *)mmap(
            NULL, sizeof(struct channel_buffer) * RX_BUFFER_COUNT,
...
void DMA_send(uint8_t *buffer, int length) {
    ...
    if (tx_channels[0].buf_ptr[buffer_id].status != PROXY_NO_ERROR) {
        ...
        printf("DMA send frame fail.\r\n");
        exit(EXIT_FAILURE);
    }
}

/* return true if this frame is ptp frame
 * Actually, for CaaS Switch at this version, all packets received by time-sync
 * DMA are ptp frames.
 */
int is_ptp_frame(uint8_t *buffer_ptr) {
    static uint8_t mac_addr_ptp[] = {0x01, 0x80, 0xc2, 0x00, 0x00, 0x0e};
    // the received buffer contains an extra header
    if (memcmp(mac_addr_ptp, buffer_ptr + CPU_HEADER_LENGTH, 6) == 0) {
...
void process_packet(uint8_t *buf, buffer_queue *queue) {
    ...
    // push_queue(queue, buf, MAX_PKT_LEN);
    push_queue(queue, buf, 80);
    pthread_mutex_unlock(&pkt_queue_lock);
    // printf("dma-proxy.c: Leaving critical region for buffer_queue.\r\n");
}

// cite:
// https://github.com/Horacehxw/software-prototypes/blob/master/linux-user-space-dma/Software/User/dma-proxy-test.c
void *DMA_rx_thread(buffer_queue *queue) {
    ...
    for (buffer_id = 0; buffer_id < RX_BUFFER_COUNT;
         buffer_id += BUFFER_INCREMENT) {
        /* Don't worry about initializing the receive buffers as the pattern
         * used in the transmit buffers is unique across every transfer so it
         * should catch errors.
         */
    ...
    /* Finish each queued up receive buffer and keep starting the buffer over
     * again until all the transfers are done
     */
    while (1) {
        // poll (waste CPU)
        ...
        if (channel_ptr->buf_ptr[buffer_id].status != PROXY_NO_ERROR) {
            //			printf("Proxy rx transfer haven't finished, # RX
            // completed %d, # in progress %d\n", rx_counter,
            // in_progress_count);
        ...
        /* Keep track how many transfers are in progress so that only the
         * specified number of transfers are attempted
         */
        in_progress_count--;
        ...
        /* Flip to next buffer treating them as a circular list, and possibly
         * skipping some to show the results when prefetching is not happening
         */
        buffer_id += BUFFER_INCREMENT;
