...
void set_ptp_frame_header(PTPFrameHeader *frame_header, PTPMsgHeader *header) {
    frame_header->majorSdoId_messageType =
        (header->majorSdoId << 4) | header->messageType;
    frame_header->minorVersionPTP_versionPTP =
        (header->minorVersionPTP << 4) | header->versionPTP;
    ...
    memcpy(frame_header->sourcePortIdentity.clockIdentity,
           header->sourcePortIdentity.clockIdentity, 8);
    frame_header->sourcePortIdentity.portNumber =
...
void get_ptp_msg_header(PTPFrameHeader *frame_header, PTPMsgHeader *header) {
    ...
    header->minorVersionPTP =
        (frame_header->minorVersionPTP_versionPTP >> 4) & 0xF;
    ...
    memcpy(header->sourcePortIdentity.clockIdentity,
           frame_header->sourcePortIdentity.clockIdentity, 8);
    header->sourcePortIdentity.portNumber =
        ntohs(frame_header->sourcePortIdentity.portNumber);
    header->sequenceId = ntohs(frame_header->sequenceId);
    header->controlField = frame_header->controlField;
    header->logMessageInterval = frame_header->logMessageInterval;
}