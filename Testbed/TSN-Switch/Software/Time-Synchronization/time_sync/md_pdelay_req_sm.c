...
static PTPMsgPdelayReq *setPdelayReq(MDPdelayReqSM *sm) {
    ...
    memcpy(portId.clockIdentity, sm->perPTPInstanceGlobal->thisClock, sizeof(ClockIdentity));
...
static void txPdelayReq(MDPdelayReqSM *sm) {
    ...
    send_ptp_frame((uint8_t *)&ptpFramePdelayReq, sizeof(PTPFramePdelayReq),
    ...
    if (uscaledns_compare(elapsedTime, sm->mdEntityGlobal->pdelayReqInterval) >=
        ...
        if (sm->rcvdPdelayResp) {
            ...
            if (sm->rcvdPdelayRespPtr->head.sequenceId ==
                ...
                log_warn("Enter PD_REQ_RESET due to infor mismatch. (rcvd vs tx) Seq ID: %04X vs %04X, Port Number: %04X vs %04X, ClockID: %02X%02X%02X%02X%02X%02X%02X%02X vs %02X%02X%02X%02X%02X%02X%02X%02X",
                        sm->rcvdPdelayRespPtr->head.sequenceId,
                        sm->txPdelayReqPtr->head.sequenceId,
                        sm->rcvdPdelayRespPtr->requestingPortIdentity.portNumber,
                        sm->perPortGlobal->thisPort,
                        sm->rcvdPdelayRespPtr->requestingPortIdentity.clockIdentity[0],
                        sm->rcvdPdelayRespPtr->requestingPortIdentity.clockIdentity[1],
                        sm->rcvdPdelayRespPtr->requestingPortIdentity.clockIdentity[2],
                        sm->rcvdPdelayRespPtr->requestingPortIdentity.clockIdentity[3],
                        sm->rcvdPdelayRespPtr->requestingPortIdentity.clockIdentity[4],
                        sm->rcvdPdelayRespPtr->requestingPortIdentity.clockIdentity[5],
                        sm->rcvdPdelayRespPtr->requestingPortIdentity.clockIdentity[6],
                        sm->rcvdPdelayRespPtr->requestingPortIdentity.clockIdentity[7],
                        sm->perPTPInstanceGlobal->thisClock[0],
                        sm->perPTPInstanceGlobal->thisClock[1],
                        sm->perPTPInstanceGlobal->thisClock[2],
                        sm->perPTPInstanceGlobal->thisClock[3],
                        sm->perPTPInstanceGlobal->thisClock[4],
                        sm->perPTPInstanceGlobal->thisClock[5],
                        sm->perPTPInstanceGlobal->thisClock[6],
                        sm->perPTPInstanceGlobal->thisClock[7]
                       );
                return PD_REQ_RESET;
            }
        } else {
...
void md_pdelay_req_sm_txts(MDPdelayReqSM *sm, UScaledNs ts,
    ...
    if (sm->state != PD_REQ_SEND_PDELAY_REQ &&
        ...
        log_error("Timestamp is not expected by MDPdelayReqSM.");
        return;
    }
    if (tsuTimestamp.sequenceID != sm->pdelayReqSequenceId) {
        log_error("Mismatched sequence ID.");
        return;
    }
