...
static PortSyncSync *setPSSyncPSSR(PortSyncSyncReceiveSM *sm, UScaledNs ts) {
    ...
    /* perPTPInstanceGlobal->syncReceiptLocalTime point to the value of the syncReceiptTimeoutTime
       member of the most recently received PortSyncSync structure. from 10.2.4.25
    */
    memcpy(&sm->perPTPInstanceGlobal->syncReceiptTimeoutTime, &pss_ptr->syncReceiptTimeoutTime, sizeof(UScaledNs));
    
    // log_warn("in pss syncReceiptTimeoutTime: is set to: [0x%016" PRIX64 "] ns", sm->perPTPInstanceGlobal->syncReceiptTimeoutTime.nsec);
...
static void received_sync_action(PortSyncSyncReceiveSM *sm, UScaledNs ts) {
    ...
    int TEMP = sm->rcvdMDSyncPtrPSSR->logMessageInterval;
    // log_warn("sm->rcvdMDSyncPtrPSSR->logMessageInterval: %d", sm->rcvdMDSyncPtrPSSR->logMessageInterval);
    sm->perPortGlobal->syncReceiptTimeoutTimeInterval.subns = 0;
    sm->perPortGlobal->syncReceiptTimeoutTimeInterval.nsec_msb = 0;
    sm->perPortGlobal->syncReceiptTimeoutTimeInterval.nsec = (uint64_t)sm->perPortGlobal->syncReceiptTimeout * ONE_SEC_NS * (1 << TEMP);
