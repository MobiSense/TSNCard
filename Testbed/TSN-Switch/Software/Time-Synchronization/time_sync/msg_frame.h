...
typedef struct PTPFrameClockQuality {
    uint8_t clockClass;
    Enumeration8 clockAccuracy;
    uint16_t offsetScaledLogVariance;
} __attribute__((packed)) PTPFrameClockQuality;

typedef struct PTPFramePathTraceTLV {
    uint16_t tlvType;
    uint16_t lengthField;
    uint8_t pathSequence[8][8];
} __attribute__((packed)) PTPFramePathTraceTLV;

typedef struct PTPFrameAnnounce {
    PTPFrameHeader head;
    uint8_t res0[10];
    int16_t currentUtcOffset;
    uint8_t res1;
    uint8_t grandmasterPriority1;
    PTPFrameClockQuality grandmasterClockQuality;
    uint8_t grandmasterPriority2;
    uint8_t grandmasterIdentity[8];
    uint16_t stepsRemoved;
    uint8_t timeSource;
    PTPFramePathTraceTLV pathTraceTLV;
} __attribute__((packed)) PTPFrameAnnounce;
