...
static void  computeGmRateRatio(ClockMasterSyncReceiveSM *sm) {
...
static UScaledNs updateMasterTime(ClockMasterSyncReceiveSM *sm) {
    ...
    sm->perPTPInstanceGlobal->masterTime =
        sm->rcvdClockSourceReqPtr->sourceTime;
    return sm->perPTPInstanceGlobal->masterTime;
    // }
}

static ClockMasterSyncReceiveSMState all_state_transition(
    ClockMasterSyncReceiveSM *sm) {
...
static ClockMasterSyncReceiveSMState initializing_state_transition(
    ClockMasterSyncReceiveSM *sm, UScaledNs ts) {
...
static void receive_source_time_action(ClockMasterSyncReceiveSM *sm,
                                       UScaledNs ts) {
    UScaledNs current_ts = updateMasterTime(sm);
    sm->perPTPInstanceGlobal->localTime = current_ts;
    if (sm->rcvdClockSourceReq) {
        computeGmRateRatio(sm);
        sm->perPTPInstanceGlobal->clockSourceTimeBaseIndicatorOld =
            sm->perPTPInstanceGlobal->clockSourceTimeBaseIndicator;
        sm->perPTPInstanceGlobal->clockSourceTimeBaseIndicator =
            sm->rcvdClockSourceReqPtr->timeBaseIndicator;
        sm->rcvdClockSourceReqPtr->lastGmPhaseChange =
            sm->rcvdClockSourceReqPtr->lastGmPhaseChange;
        sm->rcvdClockSourceReqPtr->lastGmFreqChange =
            sm->rcvdClockSourceReqPtr->lastGmFreqChange;
    }
    sm->rcvdClockSourceReq = 0;
    sm->rcvdLocalClockTickCMSR = 0;
...
static ClockMasterSyncReceiveSMState receive_source_time_state_transition(
    ClockMasterSyncReceiveSM *sm, UScaledNs ts) {
    if (sm->rcvdClockSourceReq || sm->rcvdLocalClockTickCMSR) {
...
void clock_master_sync_receive_sm_run(ClockMasterSyncReceiveSM *sm,
                                      UScaledNs ts) {
    bool state_change;
...
void init_clock_master_sync_receive_sm(
    ClockMasterSyncReceiveSM *sm,
    PerPTPInstanceGlobal *per_pip_instance_global) {
...
void clock_master_sync_receive_sm_recv_source_time(
    ClockMasterSyncReceiveSM *sm, ClockSourceTimeInvoke *source_time_req,
    UScaledNs ts) {
