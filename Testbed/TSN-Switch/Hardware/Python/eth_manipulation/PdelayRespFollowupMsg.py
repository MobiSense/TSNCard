class PdelayRespFollowupMsg:
    def __init__(self, sequence_id_bytes, resp_origin_ts, req_clock_identity, req_port_bytes):
        self.dst_addr = b'\x01\x80\xc2\x00\x00\x0e'
        self.src_addr = b'\x01\x02\x03\x04\x05\x07'
        self.ether_type = b'\x88\xf7'
        self.major_msgtype = b'\x1a'
        self.version = b'\x02'
        ...
        self.resp_origin_ts_sec_msb = b'\x00\x00'
        ts_sec = resp_origin_ts // 1000000000
        ts_ns = resp_origin_ts % 1000000000
        self.resp_origin_ts_sec_lsb = ts_sec.to_bytes(4, "big")
        self.resp_origin_ts_ns = ts_ns.to_bytes(4, "big")
    ...
    def from_bytes(self, frame_bytes):
        ...
        if self.major_msgtype & 0xf != 0xa:
            return False
        self.sequence_id = frame_bytes[44:46]
        self.seq_id = int.from_bytes(self.sequence_id, 'big')
        self.resp_origin_ts_sec_lsb = frame_bytes[50:54]
        ts_sec = int.from_bytes(self.resp_origin_ts_sec_lsb, 'big')
        self.resp_origin_ts_ns = frame_bytes[54:58]
        ts_ns = int.from_bytes(self.resp_origin_ts_ns, 'big')
        self.t3 = ts_sec * 1000000000 + ts_ns
        return True
    ...
    def to_bytes(self):
        ...
        frame_bytes += self.resp_origin_ts_sec_msb
        frame_bytes += self.resp_origin_ts_sec_lsb
        frame_bytes += self.resp_origin_ts_ns
